...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$app=$env:DORKA_APP_DIR; $desktop=[Environment]::GetFolderPath('Desktop'); $iconFile=Join-Path $app 'dorkA.ico'; $package=Get-ChildItem -LiteralPath (Join-Path $app 'release') -Filter 'dorkA *.exe' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1; $target=if($package){$package.FullName}else{Join-Path $app 'start-dorka.bat'}; $shell=New-Object -ComObject WScript.Shell; $shortcut=$shell.CreateShortcut((Join-Path $desktop 'dorkA.lnk')); $shortcut.TargetPath=$target; $shortcut.WorkingDirectory=$app; $shortcut.IconLocation=($iconFile + ',0'); $shortcut.Description='Open the dorkA desktop search workspace'; $shortcut.Save()"
